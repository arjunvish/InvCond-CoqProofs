...
(*  Lemma inv_bvadd: forall (n : N), forall (s t : bitvector n), 
  iff (exists (x : bitvector n), (bv_add x s = t)) True.
 Proof. intros n a b.
         unfold bv_eq. simpl.
         rewrite bv_existsn. 
         unfold bv_add. simpl.
         destruct a as (a, wfa). cbn.
         destruct b as (b, wfb). cbn. 
 
         specialize (@bv_exists n); intros.
         specialize (bv_exists .
         unfold bv_add. simpl. destruct a, b. cbn.
         rewrite M.inv_bvadd.
         destruct a, b.
         destruct a, b. simpl. 
         unfold bv_add. cbn. erewrite M.inv_bvadd. easy.
...
Fixpoint bv2nat_2_aux (b : bitvector) (exp : nat) : nat :=
  match b with
  | [] => 0
  | h :: t => if h then (pow2 exp) + bv2nat_2_aux t (exp + 1)
                   else bv2nat_2_aux t (exp + 1)
  end.

Definition bv2nat_2 (b : bitvector) : nat :=
  bv2nat_2_aux b 0.

Fixpoint bv2nat_2_rev_aux (b : bitvector) (exp : nat) : nat :=
  match b with
  | [] => 0
  | h :: t => if h then (pow2 exp) + bv2nat_2_rev_aux t (exp - 1)
                   else bv2nat_2_rev_aux t (exp - 1)
  end.

Definition bv2nat_2_rev (b : bitvector) : nat :=
  bv2nat_2_rev_aux (rev b) ((length b) - 1).
...
Lemma last_bv_ashr_gt0: forall t s,
  size t = size s ->
  (0 <? N.to_nat (list2N t))%nat = true -> 
  last (bv_shr_a s t) false = false.
Proof. intros.
